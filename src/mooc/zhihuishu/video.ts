...
export class ZhsVideo implements Mooc {
  protected lastTimer: NodeJS.Timeout;
  ...
  protected createToolsBar() {
    ...
    boomBtn.onclick = () => {
      ...
      let timeStr = (<HTMLSpanElement>(
        document.querySelector(".nPlayTime .duration")
      )).innerText;
      let time = 0;
      ...
      let lessonId = 0,
        smallLessonId = 0,
        chapterId = 0;
      let subTit = (<HTMLSpanElement>(
        document.querySelector(".current_play .hour")
      )).innerText;
      //获取x.x.x DOM
      let videoTag = subTit.split(".");
      if (videoTag.length >= 2) {
        lessonId =
          this.videoList.videoChapterDtos[Number.parseInt(videoTag[0]) - 1]
            .videoLessons[Number.parseInt(videoTag[1]) - 1].id;
        chapterId =
          this.videoList.videoChapterDtos[Number.parseInt(videoTag[0]) - 1]
            .videoLessons[Number.parseInt(videoTag[1]) - 1].chapterId;
    ...
    let startBtn = CssBtn(
      createBtn(startBtnText, "", "zhs-tools-btn zhs-start-btn"),
    );
  ...
  learningTimeRecord() {
    var t = parseInt((this.studyTotalTime / 5 + 2).toString()),
      e =
        null == this.watchPointPost || "" == this.watchPointPost
          ? "0,1,"
          : this.watchPointPost + ",";
  ...
  protected start() {
    let hookPlayerStarter = new Hook(
      "createPlayer",
      (<any>Application.GlobalContext).PlayerStarter,
    );
    let self = this;
    hookPlayerStarter.Middleware(function (next: Context, ...args: any) {
      ...
      let innerTimer = setInterval(function () {
        if (
          document.querySelectorAll(".current_play .time_icofinish").length > 0
        ) {
  ...
  protected hookAjax(): void {
    let hookXMLHttpRequest = new Hook(
      "open",
