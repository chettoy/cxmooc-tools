...
export class TaskFactory {
  public static CreateTask(url: string, resp: string): Task {
    if (resp.indexOf("paper:s") > 0) {
      CourseTopicTask.collegeAnswer(
        this.getvalue(resp, resp.match(/,paper:(.*?),/)[1]),
      );
      return new CourseTopicTask();
    } else if (resp.indexOf('tname:"') > 0) {
      if (resp.indexOf("answers:s") > 0) {
        CourseTopicTask.collegeAnswer(
          this.getvalue(resp, resp.match(/,objectiveQList:(.*?),/)[1]),
        );
      }
      ...
      resp.indexOf("list:s") > 0 &&
      url.indexOf("PostBean.getPaginationReplys") > 0
    ) {
      return new DiscussTask(
        this.getvalue(resp, resp.match(/,list:(.*?),/)[1]),
      );
...
export class CourseTopicTask extends Task {
  ...
  public static collegeAnswer(resp: any) {
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      for (let i = 0; i < topic.optionDtos.length; i++) {
        let opt = {
          content: topic.optionDtos[i].content,
  ...
  public Init(): Promise<void> {
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        ...
        let search = createBtn(
          "搜索答案",
          "点击搜索答案",
          "cx-btn mooc163-search",
          "tools-search",
        );
  ...
  public Start(): Promise<any> {
    return new Promise<any>(async (resolve) => {
      protocolPrompt(
        "你正准备使用中国慕课的答题功能,相应的我们需要你的正确答案,同意之后扩展将自动检索你的所有答案\n* 本项选择不会影响你的正常使用(协议当前版本有效)\n* 手动点击答题结果页面自动采集页面答案\n",
        "course_answer_collect_v2",
        "我同意",
      );
    ...
    return new Promise<void>((resolve) => {
      ...
      let timer = setInterval(() => {
        try {
          ...
          (<any>Application.GlobalContext).UE.instants[
            "ueditorInstant" + el.id.substr(el.id.indexOf("_") + 1)
  ...
  public Submit(): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
      ...
      let el = <HTMLButtonElement>(
        document.querySelector(".u-btn-sm.u-btn-primary")
      );
      if (!el) {
        return resolve();
      }
