        ...
        if (task) {
          setTimeout(async () => {
            ...
            if (Application.App.config.auto) {
              let autonext = <HTMLInputElement>(
                document.querySelector(".j-autoNext")
              );
  ...
  protected nextTask() {
    ...
    let tmp = (type: string) => {
      ...
      let all = document.querySelectorAll(
        ".f-fl.j-" + type + " .f-bg.j-list > .f-thide",
      );
      return this.next(all, (el) => {
        //什么魔鬼,空格不同
        return (
          (<HTMLSpanElement>el).innerText.replace(/\s/g, "") ==
          (<HTMLSpanElement>now).innerText.replace(/\s/g, "")
        );
      });
    };
    ret = tmp("lesson");
    ...
    if (ret) {
      UntrustedClick(ret);
      let all = document.querySelectorAll(
        ".f-fl.j-lesson .f-bg.j-list > .f-thide",
      );
